...
export function autoMaxDraws(targets: Targets) {
  ...
  return desiredTotal * 200;
}

export function baseCategoryProbs(
  hasAnnouncer: boolean,
...
export function resourcesToDraws(r: Resources) {
  const tenFromLunacy = Math.floor(r.lunacy / 1300);
  const remainder = r.lunacy % 1300;
  const oneFromLunacy = Math.floor(remainder / 130);
...
export function cumulativeSuccess(
  ...
  for (let i = 0; i < r && i < pityAlloc.length; i++)
    pityCounts[pityAlloc[i]]++;
  ...
  const { pA, pE, pT } = wantProbPerCategory(
    settings.hasAnnouncer,
    true,
    targets
  );
...
export function computeGreedyPityAlloc(
  ...
  for (let r = 1; r <= R; r++) {
    ...
    if (remaining.A > 0)
      opts.push({ cat: "A", gain: binomPMF(remaining.A - 1, n, pA) });
    if (remaining.E > 0)
      opts.push({ cat: "E", gain: binomPMF(remaining.E - 1, n, pE) });
    if (remaining.T > 0)
      opts.push({ cat: "T", gain: binomPMF(remaining.T - 1, n, pT) });
    if (!opts.length) {
      alloc.push("E");
      continue;
    }
